 ...
 */
  ...
  */
  ...
  /*
    Suite à un message [GLIS?***], envoie [GLIS!␣s***] puis s fois [GPLYR␣id␣x␣y␣p***]
  */
  public void treatGList(PrintWriter pw, Game g) throws IOException {
    try{
      String s = check_number(String.valueOf(g.getPlayers().size()));
      String message = "GLIS! " + s + "***";
      sendTCP(pw, message);

      Iterator<Player> itp = g.getPlayers().iterator();
      while(itp.hasNext()){
        Player p = itp.next();
        String gl_msg = "GPLYR "+p.getId()+" "+checkPos(p.getX())+" "+checkPos(p.getY())+" "+checkScore(p.getScore())+"***";
        sendTCP(pw, gl_msg);
      }
    } catch (Exception e) {
      e.printStackTrace();
  ...
  /*
    Après un [UPMOV/DOMOV/RIMOV/LEMOV_d***]
    - on bouge le joueur et on renvoie soit [MOVE!␣x␣y***] ou soit [MOVEF␣x␣y␣p***]
  */
  public boolean treatMove(PrintWriter pw, DatagramSocket dso, Game g, Player p, String message){
    try{
      ...
      if(tmp < p.getScore()){
        sendTCP(pw, "MOVEF "+checkPos(p.getX())+" "+checkPos(p.getY())+" "+checkScore(p.getScore())+"***");
        //[SCORE␣id␣p␣x␣y+++]
        if(ghosts_pos != null){
          /* Pour chaque position dans coordinates on envoie un message multi-diffusé */
          for(int i = 0 ; i < ghosts_pos.size() ; i++){
            String scor = "SCORE "+p.getId()+" "+checkScore(p.getScore())+" "+checkPos(ghosts_pos.get(i).getX())+" "+checkPos(ghosts_pos.get(i).getY())+"+++";
            sendPacket(dso, InetAddress.getByName(g.getIpAddress()), g.getMulticastPort(), scor);
          }
        }

        /* Si l'on a plus de fantômes dans la partie, on multi-diffuse ENDGA */
        if(g.getGhosts().size() == 0){
            Player best_player = g.getWinner();
            String end = "ENDGA "+best_player.getId()+" "+checkScore(best_player.getScore())+"+++";
            sendPacket(dso, InetAddress.getByName(g.getIpAddress()), g.getMulticastPort(), end);

            return true;
        }
      } else {
        sendTCP(pw, "MOVE!"+checkPos(p.getX())+" "+checkScore(p.getY())+"***");
      }
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  */
  public void treatGames(PrintWriter pw) throws IOException {
    ...
    while(itg.hasNext()){
      ...
      if(!g.getStart()){
        ...
        String res2 = "OGAME " + check_number(numb) + " " + check_number(len) + "***";
        sendTCP(pw, res2);
      }
    }
  }
  ...
  /*
    Suite à [SEND?␣id␣mess***] renvoie [SEND!***] après avoir envoyé en udp [MESSP␣id2␣mess+++]
  */
  public void treatSend(PrintWriter pw, DatagramSocket dso, Player p, Game g, String message, InetAddress ia) throws IOException {
    ...
    if(exist_in_game){
      String to_send = "MESSP "+p.getId()+" "+mess+"+++";
  ...
  /*
  Suite à un message [QUIT?␣m***], renvoie [GOBYE***]
  */
  ...
  /*
    Suite à [MALL?␣mess***] envoie [MALL!***]
    après avoir envoyé [MESSA␣id␣mess+++] aux joueurs de la partie
  */
  void treatAll(PrintWriter pw, DatagramSocket dso, String id_send, String m, int port, String ip) throws IOException {
    ...
    String message = "MESSA "+id_send+" "+msg+"+++";
    sendPacket(dso, InetAddress.getByName(ip), port, message);
    sendTCP(pw, "MALL!***");
  }

  /*
    Suite à [START***], envoie [WELCO␣m␣h␣w␣f␣ip␣port***]
  */
      ...
      if(p.getStart() == false){
        ...
        while(not_send){
          ...
          if(g.allPlayersReady()){
            ...
            sendTCP(pw, "WELCO "+m+" "+h+" "+w+" "+f+" "+ip+" "+port+"***");
            int x = p.getX();
            int y = p.getY();
            sendTCP(pw, "POSIT "+p.getId()+" "+checkPos(x)+" "+checkPos(y)+"***");
            not_send = false;
          }
        }
      } else {
