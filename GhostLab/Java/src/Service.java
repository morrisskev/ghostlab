...
public class Service implements Runnable {
  ...
  public void launch(PrintWriter pw, BufferedReader br, DatagramSocket dso, Socket socket){
      try {
        server.treatGames(pw); //[GAMES n***] puis [OGAME s m***] attente

        /* Permet de fermer la connexion depuis un message envoyé après le ENDGA */
        int i = 0;
        ...
        while(true){
          ...
          switch(type){
            ...
            case "UPMOV": //[UPMOV␣d***] [MOVE!␣x␣y***]
              if(g.getEnd()){
                server.sendTCP(pw, "GOBYE***");
                if(this.p != null){
            ...
            case "GLIS?***": //[GLIS?***] puis renvoie [GLIS!␣s***] puis [GPLYR␣id␣x␣y␣p***] s fois
              if(g.getEnd()){
                server.sendTCP(pw, "GOBYE***");
                if(this.p != null){
                  g.removePlayer(this.p);
            ...
            case "MALL?": //[MALL?␣mess***] renvoie [MALL!***] après avoir multi-diffusé
              if(g.getEnd()){
                server.sendTCP(pw, "GOBYE***");
                if(this.p != null){
                  g.removePlayer(this.p);
