...
public class Player {
	...
	String before_start(PrintWriter pw, BufferedReader br) throws Exception {
		...
		while(loop){
			...
			if(tab.length == 1){
				if(str.equals("/new")){ 	// le cleint envoie [NEWPL id port***]
					String nw = "NEWPL "+this.id+" "+this.port_udp+"***";
					writeInfo(pw, nw);
					readInfo(br);
				}
				...
				if(str.equals("/start")){ // demander la deconnxion
					...
					String nw = "REGIS "+this.id+" "+this.port_udp+" "+number+"***";
	...
	void after_start(BufferedReader br, PrintWriter pw) throws IOException {
		String pos = readInfo(br); //[POSIT␣id␣x␣y***]
				...
				if(cmd.length == 2){
					...
					if(Pattern.matches("/(u|d|r|l)\\b", direction)){
						...
						if(Pattern.matches("\\d{1,3}\\b", dist_m)){
							...
							switch(direction){  // [UPMOV␣d***],[DOMOV␣d***], [LEMOV␣d***] et [RIMOV␣d***]
								case "/u":
								to_send = "UPMOVE!"+s_dist+"***";
								break;

								case "/d":
								to_send = "DOMOVE!"+s_dist+"***";
								break;

								case "/l":
								to_send = "LEMOVE!"+s_dist+"***";
								break;

								case "/r":
								to_send = "RIMOVE!"+s_dist+"***";
								break;
							}

							writeInfo(pw, to_send);
							if(readInfo(br).equals("GOBYE***"))//Reçoit [MOVE!␣x␣y***] ou [MOVEF x y p***]
								loop = false;
						}
					} else if(cmd[0].equals("/all")){
						String msg = str.substring(5, str.length());
						if(checkStars(msg) && checkPlus(msg)){
							writeInfo(pw,"MALL? "+str.substring(5, str.length())+"***");  // [MALL?␣mess***]
							if(readInfo(br).equals("GOBYE***"))// Reçoit [SEND!***] ou [NSEND***]
								loop = false;
						}
					}
					...
					switch(str){
						case "/quit" :
							writeInfo(pw, "IQUIT***");
							readInfo(br); // reçois [GOBYE***]
							loop = false;
							break;

						case "/players" :
							writeInfo(pw, "GLIS?***");
							String s = readInfo(br); // Reçoit [GLIS!␣s***]
							String[] sp = s.split(" ");
							int nb_p = Integer.parseInt(sp[1].substring(0,2));
							for(int i = 0; i<nb_p; i++){
								if(readInfo(br).equals("GOBYE***"))// Reçoit [GPLYR␣id␣x␣y␣p***]
									loop = false;
							}
							break;
					}
	...
	String treat_start(BufferedReader br) throws Exception { //S = welcome
		...
		String s = readInfo(br); //[WELCO␣m␣h␣w␣f␣ip␣port***]
		if(!s.equals("DUNNO***")){
			return s;
		}
		return s;
	...
	public static void main(String[] args) {
		...
		try {
			...
			String[] tab_welcome = welcome_message.split(" "); //[WELCO␣m␣h␣w␣f␣ip␣port***]
